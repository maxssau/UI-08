    ...
    if( play_session->alternate  ==  0)
    {
      if (GetAudioConfiguration() != AUDIO_CONFIG_2_0_STEREO_32_BIT)
      {
        switch(alternate)
        {
          ...
          case ALTERNATE_SETTING_24_BIT:
            play_session->session.node_list->audio_description->resolution = CONFIG_RES_BYTE_24;
          break;
        }
...
/************************ (C) COPYRIGHT STMicroelectronics *****END OF FILE****/
 ...
 void UpdateInputNodePackLength(void)
 {
  ...
  #else
    PlaybackUSBInputNode.max_packet_length = AUDIO_USB_MAX_PACKET_SIZE_FROM_AUD_DESC(&PlaybackAudioDescription);
  #endif /* USE_AUDIO_PLAYBACK_USB_FEEDBACK */
 }
