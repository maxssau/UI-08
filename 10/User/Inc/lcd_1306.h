...
void sendCommand(uint8_t command_s)
{
	...
	temp[1] = command_s;
        int k=0;
	while(HAL_I2C_Master_Transmit(&hi2c3,OLED_adress,temp,2,LCD_TIMEOUT)!=HAL_OK)
        {
...
void LCD_Char(unsigned int c)
{
	...
	temp_char[6] = 0;
        int k=0;
	while(HAL_I2C_Master_Transmit(&hi2c3, OLED_adress, temp_char, 7,LCD_TIMEOUT)!=HAL_OK)
        {
          __HAL_RCC_I2C3_FORCE_RESET();
          k++;
          __HAL_RCC_I2C3_RELEASE_RESET();
...
void I2C3_Init(void)
{
	...
	*/
	...
	I2C3->CR1 &= ~I2C_CR1_SMBUS;
	
	I2C3->CR2 &= ~I2C_CR2_FREQ;
	I2C3->CR2 |= 42; // Fclk1=168/4=42MHz 
		
	I2C3->CCR &= ~(I2C_CCR_FS | I2C_CCR_DUTY);
        //?????? ??????? ?????? ?????? SCL ?? ??????? 10 000nS/(2* TPCLK1) 
	I2C3->CCR |= 208; //10 000ns/48ns = 208
	
	//Standart_Mode = 1000nS, Fast_Mode = 300nS, 1/42MHz = 24nS
	I2C3->TRISE = 42; //(1000nS/24nS)+1
...
void I2C_Write(uint8_t reg_addr, uint8_t data)
{
	...
	(void) I2C3->SR1;
        ...
        //???????? ????? ??????????
	I2C3->DR = I2C_ADDRESS(reg_addr,I2C_MODE_WRITE);
...
uint8_t I2C_Read(uint8_t reg_addr)
{
	...
	//???????? ????? ??????????, ?? ?????? ??? ??????
	I2C3->DR = I2C_ADDRESS(reg_addr,I2C_MODE_READ);
	while(!(I2C3->SR1 & I2C_SR1_ADDR)){};
	(void) I2C3->SR1;
	(void) I2C3->SR2;
