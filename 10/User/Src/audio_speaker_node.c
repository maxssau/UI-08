...
void BSP_AUDIO_OUT_TransferComplete_CallBack(void)
{
  ...
  if(AUDIO_SpeakerHandler->specific.cmd & SPEAKER_CMD_CHANGE_RESOLUTION)
  {
    AUDIO_SpeakerHandler->node.state = AUDIO_NODE_STOPPED;
    AUDIO_SpeakerInitInjectionsParams( AUDIO_SpeakerHandler);    
    AudioChangeResolution(AUDIO_SpeakerHandler->node.audio_description->resolution<<3);
    AUDIO_SpeakerHandler->specific.cmd &= ~SPEAKER_CMD_CHANGE_RESOLUTION;
  }
    
    AUDIO_SpeakerHandler->SpeakerPlay((uint16_t*)AUDIO_SpeakerHandler->specific.data,
...
//#if (USB_AUDIO_CONFIG_PLAY_RES_BIT == 24)
      if (AUDIO_SpeakerHandler->node.audio_description->resolution == CONFIG_RES_BYTE_24)
      {
...
//#endif /*  USB_AUDIO_CONFIG_PLAY_RES_BIT */
        ...
        if (AUDIO_SpeakerHandler->node.audio_description->channels_count > 2)
          AUDIO_SpeakerHandler->SpeakerPrepareData(AUDIO_SpeakerHandler->specific.data,
                                      (uint16_t)AUDIO_SpeakerHandler->specific.data_size,
                                      AUDIO_SpeakerHandler->node.audio_description->resolution);
...
static void  AUDIO_SpeakerInitInjectionsParams( AUDIO_SpeakerNode_t* speaker)
{
  ...
  speaker->specific.injection_size = AUDIO_MS_PACKET_SIZE((speaker->node.audio_description)->frequency, (speaker->node.audio_description)->channels_count, (speaker->node.audio_description)->resolution);
  speaker->specific.double_buff = 0;
  speaker->specific.offset = 0;
//#if USB_AUDIO_CONFIG_PLAY_RES_BIT == 24
  if (speaker->node.audio_description->resolution == CONFIG_RES_BYTE_24)
  {
    speaker->specific.injection_size = AUDIO_MS_PACKET_SIZE((speaker->node.audio_description)->frequency, (speaker->node.audio_description)->channels_count, 4);
...
static int8_t  AUDIO_SpeakerStartReadCount( uint32_t node_handle)
{
    ...
    speaker = (AUDIO_SpeakerNode_t*)node_handle;
    speaker->specific.dma_remaining = SAI_GetRemainingTxSize();//SAI_MASTER_DMA_STREAM->NDTR;//__HAL_DMA_GET_COUNTER(haudio_out_sai_1.hdmatx);
    return 0;    
}
...
static uint16_t  AUDIO_SpeakerGetLastReadCount( uint32_t node_handle)
{
  ...
  int cur_waiting_bytes, read_bytes, last_packet_size;
   ...
   /* read remind value in dma buffer */
    cur_waiting_bytes = SAI_GetRemainingTxSize();//SAI_MASTER_DMA_STREAM->NDTR; //__HAL_DMA_GET_COUNTER(haudio_out_sai_1.hdmatx);
    last_packet_size = SAI_GetLastTransferSize();//haudio_out_sai_1.XferSize;
