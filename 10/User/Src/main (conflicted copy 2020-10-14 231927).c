...
#define SCL_I HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_8);   
#define SDA_I HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_4);
#define SCL_O HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, GPIO_PIN_RESET);
#define SDA_O HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_RESET);
//--------------------------------------------------------------------------------
void i2c_init (void);               // ????????????? ????
...
//char DAC_InitData[]={166,2,1,255,255,1,0,0,0,0};
...
void SIGMA_WRITE_REGISTER_BLOCK(int devAddress, int address,unsigned int length, uint8_t *pData)
{
  ...
  if(i2c_send_byte((address & 0xFF00)>>8))
  {
    err_success_writes++;
  }
  else
...
int main(void)
{
  ...
  while (1)
  {
    ...
    */
    ...
    {
      if(BTN_ctrl_tick_last-HAL_GetTick()>uDSP01_Ctrl_Button_Timeout)
      {
    ...
    if(__encoder_data!=__encoder_data_last)
    {
      if(Screen_Current==0)
      {
        ...
        DSP_write_volume_r(dsp_volume_r);
        DSP_write_volume_l(dsp_volume_l);
...
void Screen_DrawVolume()
{
  ...
  if(dsp_volume_r==0)
  {
    sprintf(__buff,"  %.1f db",dsp_volume_r);
  }
  else
  {
    if(dsp_volume_r<-9.9)
...
void DAC_Update()
{
  ...
  {
    i2c_start_cond();
    if(i2c_send_byte(DAC_I2C_ADDRESS))
    {
       err_success_writes++;
    }
    if(i2c_send_byte(i))
    ...
    if(i2c_send_byte(DAC_InitData[i]))
    {
      err_success_writes++;
    }
    i2c_stop_cond();
